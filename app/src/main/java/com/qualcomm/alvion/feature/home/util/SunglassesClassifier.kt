...
class SunglassesClassifier(
    ...
    fun classify(faceBitmap: Bitmap): ClassificationResult {
        ...
        return try {
            ...
            val resizedBitmap =
                if (faceBitmap.width != INPUT_SIZE || faceBitmap.height != INPUT_SIZE) {
            ...
            Log.d(
                TAG,
                "Classification: raw=${"%.3f".format(
                    rawOutput,
                )}, sunglassesConfidence=${"%.3f".format(confidence)}, sunglasses=$hasSunglasses",
            )
