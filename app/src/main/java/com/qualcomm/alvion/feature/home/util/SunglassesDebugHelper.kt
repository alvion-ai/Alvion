...
object SunglassesDebugHelper {
    ...
    fun logFrameProcessing(frameNumber: Int, runInference: Boolean) {
    ...
    fun logImageExtraction(success: Boolean, width: Int = 0, height: Int = 0) {
        if (debugLoggingEnabled) {
            if (success) {
    ...
    fun logInferenceResult(confidence: Float, hasSunglasses: Boolean) {
        if (debugLoggingEnabled) {
            val message = if (hasSunglasses) {
    ...
    fun logStateChange(detected: Boolean, reason: String) {
        if (debugLoggingEnabled) {
            if (detected) {
