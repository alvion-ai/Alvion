...
\begin{itemize}
    ...
    The system shall utilize the ML Kit Face Detection API [1] to analyze eye-openness probabilities. Using the PERCLOS metric [4], it shall trigger an alert when eyelid closure exceeds a defined duration (default 1800ms).
...
\begin{itemize}
    \item User Functional Requirements (UF-A through UF-J)
    \item System Functional Requirements (SF-01 through SF-10)
    \item Design components such as the FaceDetectionAnalyzer and FaceStateEvaluator
...
\textbf{What it is.} A secure cloud-based identity platform [3].
...
\begin{itemize}
    ...
    \item \textbf{Controllers (Analysis Pipeline):} The \texttt{FaceDetectionAnalyzer} acts as the primary controller. It coordinates the data flow between the \textbf{CameraX [2]} frame stream and the \textbf{ML Kit [1]} inference engine. It ensures that frames are processed asynchronously to maintain a responsive UI.
...
\begin{itemize}
    ...
    \item \textbf{UF-I (Session Summary):} Basic session summary functionality implemented for user feedback.

    \item \textbf{UF-J (Privacy Control):} All processing is performed on-device, ensuring no transmission of raw facial data.
\end{itemize}

All functional requirements are successfully implemented and validated through both automated and manual testing.
...
\section{Achievement}
\begin{itemize}
    \item \textbf{Heuristic Accuracy:} Implemented a logic engine capable of $1800$ms drowsiness detection and tiered distraction timeouts based on subject-specific calibration.
    ...
    \item \textbf{Cloud Integration:} Successfully integrated Firebase for secure authentication and alert history, providing a complete "product" experience for the end user.
\end{itemize}
...
\includepdf[pages=-, fitpaper=true]{assets/CSU-SM-CSE-39-2026-SE-001-Team-009-AllInOne (10).pdf}
